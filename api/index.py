...
# Regular expressions to extract subtotal, tax, and total (allowing missing letters anywhere)
    ...
    tax_pattern = r'ta?x\s*\$?(\d+\.\d{2})'  # Handle variations of 'tax'  # Handle variations of 'total'
    ...
    items = {item: details for item, details in items.items() if details['price'] != total and not re.search(r'(total|subtotal|tax)', item, re.IGNORECASE)}
